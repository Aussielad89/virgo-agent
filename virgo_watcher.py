...
class FileWatcher:
    ...
    def start(self, on_change: Callable[[list[str]], None]) -> None:
        ...
        print(
            f"{icon('watch')} Watching {self.base_path} "
    ...
    def _walk(self) -> list[Path]:
        ...
        for root_str, dirs, files in os.walk(str(self.base_path)):
            ...
            dirs[:] = [
                d for d in dirs if d not in self.exclude and not any(p in d for p in self.exclude)
            ]
            for fname in files:
                fpath = root / fname
...
def run_pipeline(
    ...
    *,
    ...
    print(f"  {icon('done')} Pipeline done — {len(files)} file(s) in {state.phase or '?'} phase")
...
def main() -> None:
    ...
    p.add_argument("--dir", "-d", default=".", help="Directory to watch (default: .)")
    p.add_argument(
        "--goal",
        "-g",
    ...
    p.add_argument(
        "--exclude", "-x", action="append", default=[], help="Additional exclude pattern"
    )
    p.add_argument("--iterations", type=int, default=3, help="Max WTF iterations (default: 3)")
    p.add_argument("--llm", action="store_true", help="Use LLM-backed policies")
    p.add_argument("--crush", action="store_true", help="Use Crush CLI backend")
    p.add_argument("--router", default=None, help="Path to router JSON config")
    p.add_argument("--stream", action="store_true", help="Stream LLM output")
    args = p.parse_args()

    exclude_base = ["__pycache__", ".git", ".venv", "agent_env", ".virgo_memory", ".coverage"]
    ...
    def on_change(changed: list[str]) -> None:
        ...
        print(f"\n{icon('zap')} [{ts}] Change detected — {len(changed)} file(s)")
